...
// Cache OpenID configuration to avoid repeated discovery
...
// Middleware to check if user is support team
export const isSupport: RequestHandler = async (req, res, next) => {
  const user = req.user as any;
  ...
  try {
    ...
    if (userData?.role !== 'admin' && userData?.role !== 'support') {
      return res.status(403).json({ message: "Forbidden. Support access required." });
    }
